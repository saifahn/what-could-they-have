...
import { formatText, createText } from './formatText'
...
it('italicizes text with reminder text', () => {
  const original = priceOfFame.oracle_text
...
it("doesn't italicize text without reminder text", () => {
  const original = absorb.oracle_text
  const result = formatText(original).__html
...
it('italicizes ability keywords', () => {
  const original = unbreakableFormation.oracle_text
  const result = formatText(original).__html
  expect(result).toContain('<em>')
})

it("doesn't italicize non-ability keywords before an em-dash", () => {
  const original = appliedBiomancy.oracle_text
  const result = formatText(original).__html
...
describe('new format text gives an array of elements', () => {
  it('returns a paragraph for each newline', () => {
    const original = radicalIdea.oracle_text
    const result = createText(original)
    expect(result).toHaveLength(2)
  })
  // it('has italics for reminder text')
})
